...
<div class="row">
    <div class="col-lg-12 ui-sortable">
        <div class="box ui-sortable-handle" style="position: relative;">
            <header>
                <h5>新用户审核</h5> <h5>|</h5> <a href="/admin/YUser/list2"><h5>老用户审核</h5></a> <button  onclick="window.location.reload();" class="btn btn-primary" style="float:right;">刷新</button>
            </header>
                    ...
                    <div class="row">
                        <div class="col-sm-6" style="width: 100%">
                            <div class="dataTables_length" id="dataTable_length">
                                <form method="get" action="/admin/YUser/list" class="form-inline" >
                                <label style="line-height: 50px;">UID： <input class="form-control input-sm" type="text" name="uid" value="{if $uid >0}{$uid}{/if}"><button type="submit" class="btn btn-primary">查找</button>
                                    ...
                                    <span {if $gender=="1"} class="shenhe_dian" {else} class="shenhe" {/if} onclick="window.location='/admin/YUser/list?city={$city}&gender=1'" style="padding: 10px 20px; text-align: center; margin-left: 10px;">男</span></label>
                                    <span {if $city=="长沙"} class="shenhe_dian" {else} class="shenhe" {/if} onclick="window.location='/admin/YUser/list?gender={$gender}&city=长沙'" style="  padding: 10px 20px; text-align: center; margin-left: 50px;">长沙用户</span>
                                    <span {if $city=="北京 "} class="shenhe_dian" {else} class="shenhe" {/if} onclick="window.location='/admin/YUser/list?gender={$gender}&city=北京'" style="  padding: 10px 20px; text-align: center; margin-left: 50px;">北京用户</span>
                                    <span {if $city=="非长沙"} class="shenhe_dian" {else} class="shenhe" {/if} onclick="window.location='/admin/YUser/list?gender={$gender}&city=非长沙'" style="padding: 10px 20px; text-align: center; margin-left: 10px;">非长沙用户</span></label>
                                </form>
                            </div>
                        </div>
                    </div>
                                ...
                                <tr role="row" id="item_{$item.uid}" class="{if (($smarty.foreach.foo.index+1)%2)}odd{else}even{/if}" style="border-bottom: 50px solid #ddd">
                                    <td class="sorting_1">
                                       <div style=" overflow: hidden;" class="{$item.avatarcss}" >
                                        ...
                                        <div style="line-height: 35px; font-size: 20px;"">{if $item.dynamics>0}<a href="/admin/YUser/listdynamics?uid={$item.uid}">性感动态，涉黄动态，文字未通过：{$item.dynamics}条</a>{else}性感动态，涉黄动态，文字未通过：{$item.dynamics}条{/if}</div>
                                    </td>
