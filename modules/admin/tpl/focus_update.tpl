...
<div class="form-horizontal" {if $focus.type=="loginPopup"} style="display: none;" {/if} >
    ...
    <div class="form-group form-group-sm">
        ...
        <div class="col-sm-10" style="float: left; width: 320px;">
            <textarea id="text_f" type="text" name="text_f"
                   class="form-control" reg="" tip="内容说明" style="width: 600px; height: 300px;">{$focus.text}</textarea>
        </div>
...
<form rel="ajax" id="add_form" method="post" action="/admin/FocusList/update?id={$id}" class="form-horizontal"
    ...
    <div class="form-group form-group-sm" style="display: none;">
        ...
        <div class="col-sm-10">
            <select name="type">
                <option value="main" {if $focus.type=="main"} selected {/if}>广场</option>
                <option value="loginPopup" {if $focus.type=="loginPopup"} selected {/if}>登陆弹窗</option>
            </select>
        </div>
    </div>
    ...
    <div class="form-group form-group-sm">
        ...
        <div class="col-sm-10">
            ...
            <option value="1" {if $focus.action->cmd=="cmd_open_web"} selected {/if}>打开网页</option>
                <option value="2" {if $focus.action->cmd=="cmd_open_actlist"} selected {/if}>打开活动列表界面</option>
                <option value="3" {if $focus.action->cmd=="cmd_open_act"} selected {/if}>打开某个活动</option>
                <option value="4" {if $focus.action->cmd=="cmd_open_actaward"} selected {/if}>打开活动中奖清单</option>
                <option value="5" {if $focus.action->cmd=="cmd_entry_game"} selected {/if}>打开游戏</option>
            </select>
        </div>
    </div>
    ...
    <div class="form-group form-group-sm" id="anniu1">
        ...
        <div class="col-sm-10">
            <input id="url" type="text" name="url" value="{$focus.action->data->url}" class="form-control" style="width: 500px;"/>
        </div>
    </div>
    ...
    <div class="form-group form-group-sm" id="anniu3">
        ...
        <div class="col-sm-10">
            <input id="events_id" type="text" name="events_id" value="{{$focus.action->data->id}}" class="form-control" style="width: 100px;"/>
        </div>
    </div>
    ...
    <div class="form-group form-group-sm" id="anniu5">
        ...
        <div class="col-sm-10">
            <select name="gid">
                <option value="1" {if $focus.action->data->gid=="1"} selected {/if}>飞机大战</option>
            </select>
            <select name="area_id" >
                <option value="1" {if $focus.action->data->area_id=="1"} selected {/if}>战区1</option>
            </select>
        </div>
    </div>
    </div>
    <div class="form-group form-group-sm">
        ...
        <div class="col-sm-10">
            <input id="position" type="text" name="position" value="{{$focus.position}}" class="form-control" style="width: 100px;"/>
            整数，越小越靠前
        </div>
    </div>
    <div class="form-group form-group-sm" style="margin-top: 50px">
        ...
        <div class="col-sm-10">
            <input type="hidden" name="text" id="text" value="{$focus.text}"/>
            <input type="hidden" name="pic" id="pic" value="{$focus.pic}"/>
...
<script type="text/javascript">
    ...
    {if $focus.action->cmd=="cmd_open_web"}$('#anniu1').show();{/if}
    {if $focus.action->cmd=="cmd_open_actlist"}$('#anniu2').show();{/if}
    {if $focus.action->cmd=="cmd_open_act"}$('#anniu3').show();{/if}
    {if $focus.action->cmd=="cmd_open_actaward"}$('#anniu4').show();{/if}
    {if $focus.action->cmd=="cmd_entry_game"}$('#anniu5').show();{/if}
    ...
    {if $focus.pic  neq ""}
    document.getElementById('pic_xiao').style.display="block";
    document.getElementById('pic_xiao').innerHTML='<img width="100" style="margin: 5px" height="100" src="{$focus.pic}"  class="img-rounded"/>';
    {/if}
