...
<ol class="breadcrumb">
    <li class="active">被举报用户列表</li>
</ol>

<form method="get" action="/admin/UserReport/list" class="form-inline" >
    <table>
        <tr>
            <td>
...
<table class="table" id="show_table" style="font-size: 12px;margin-top: 20px;">
    ...
    {foreach from=$users item=item name=foo}
       <tr class="t_c"  style="vertical-align: middle">
            ...
            <td id="count{$item.uid}">{$item.count}</td>
            <td id="stat{$item.uid}">
                {if $item.stat=="0"}正常{/if}
                {if $item.stat=="5"}<span style="color: #ff0000;">已封号</span>{/if}
            </td>
            <td><a href="/admin/User/xiangxi?uid={$item.uid}">详细信息</a>
                <a href="/admin/UserMessageJD/list?uid={$item.uid}" target="_blank">聊天记录</a><br>
                {if $item.stat=="0"}<button  class="btn btn-primary" style="float:right;" onclick ="shenhe({$item.uid});">封闭此用户</button>{/if}
                {if $item.stat=="5"}<button  class="btn btn-primary" style="float:right;" onclick ="shenhe_che({$item.uid});">撤销封闭</button>{/if}
                <button  class="btn btn-primary" style="float:right; margin-right: 3px;" onclick ="test1({$item.uid});">投诉清零</button>
...
<script>
    function test1(uid){
       ...
       if(!a){
        ...
        //
        var URL = "/admin/UserReport/userban?uid="+uid;
